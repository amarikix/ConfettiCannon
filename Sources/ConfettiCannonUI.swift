...
//  Playground
//
//  Created by Amari Robinson on 7/18/23.
//
...
public struct ConfettiCannon: View {
    ...
    @State var finishedAnimationcount = 0
    @State var firstAppear = false
    @State var error = ""
    
    /// renders configurable confetti animaiton
    /// - Parameters:
    ///   - count: on any change of this variable the animation is run
    ...
    public init(count:Binding<Int>,
        ...
        _confettiConfig = StateObject(wrappedValue: ConfettiCannonUI.confettiConfig(
            num: num,
            shapes: shapes,
            colors: colors,
    ...
    public var body: some View {
        ZStack{
            ForEach(finishedAnimationcount..<animate.count, id:\.self){ i in
                ConfettiContainer(
                    finishedAnimationcount: $finishedAnimationcount,
        ...
        .onChange(of: count){value in
            if firstAppear{
                for i in 0...confettiConfig.repetitions{
                    ...
                    confettiConfig.repetitionInterval * Double(i)) {
...
struct ConfettiContainer: View {
    ...
    var body: some View{
        ...
        .onAppear(){
            if firstAppear{
                DispatchQueue.main.asyncAfter(deadline: .now() +
                                              confettiConfig.animationDuration) {
...
struct ConfettiView: View{
    ...
    func getDelayBeForRainAnimation() -> TimeInterval {
    ...
    var body: some View{
        ConfettiAnimationView(shape:getShape(), color:getColor(), spinDirX: getSpinDirection(), spinDirZ: getSpinDirection())
            ...
            .onAppear(){
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + getDelayBeForRainAnimation()) {
...
class confettiConfig: ObservableObject {
    ...
    var openingAngleRed:CGFloat{
        return CGFloat(openingAngle.degrees) * 180 / .pi
    }
    
    var closingAngleRed:CGFloat{
